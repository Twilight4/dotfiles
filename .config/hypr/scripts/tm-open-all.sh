...
if ! is_kitty_running; then
    ...
    hyprctl dispatch exec 'kitty -1 -T asciiquarium --class asciiquarium -e asciiquarium --transparent'
else
...
if ! is_kitty_running; then
    # Spawn 'footclient' only if it was not running
    hyprctl dispatch exec 'kitty -1 -T cava --class cava -e cava'
else
...
if ! is_kitty_running; then
    # Spawn 'footclient' only if it was not running
    hyprctl dispatch exec 'kitty -1 -T clock --class clock -e tty-clock -c -C 6 -r -s -f "%A, %B, %d"'
else
...
if ! is_kitty_running; then
    # Spawn 'footclient' only if it was not running
    hyprctl dispatch exec 'kitty -1 -T clock --class cmatrix -e cmatrix'
else
...
if ! is_kitty_running; then
    # Spawn 'footclient' only if it was not running
    hyprctl dispatch exec 'kitty -1 -T pipes --class pipes -e ~/.config/zsh/bash-scripts/pipes'
else
...
if ! is_kitty_running; then
    # Spawn 'footclient' only if it was not running
    hyprctl dispatch exec 'kitty -1 -T rain --class rain -e ~/.config/zsh/bash-scripts/rain'
else
...
if ! is_kitty_running; then
    hyprctl dispatch exec 'kitty -1 --hold -T fetch --class fetch -e fastfetch --kitty ~/pictures/screenshots/Patrick-Bateman-Profile-Pic_600x600.jpg'
else
...
# Spawn 'kitty' if it's not running, else only focus
if ! is_kitty_running; then
    hyprctl dispatch exec 'kitty -1 --class fireplace -T fireplace -e fireplace'
else
