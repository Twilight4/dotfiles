...
1. Run my installer script
   #+begin_src bash
    ...
    #+end_src
2. Add =blurls= in hyprland.conf
   #+begin_src bash
    decoration {
        blurls = emacs
    }
   #+end_src
3. Add opacity and designated workspace in =window-rules.conf= to emacs
   #+begin_src
   windowrulev2 = opacity 0.97,class:^(emacs)$
   windowrulev2 = workspace 4, class:^(emacs)$
   #+end_src
4. Add emacs daemon to =autostart.conf=
   #+begin_src bash
    exec-once=emacs --daemon
   #+end_src
5. Add emacs client to autostart in =autolaunch= script
   #+begin_src
   ...
   #+end_src
6. Set vars in =.zshenv=
   #+begin_src bash
    EDITOR="emacsclient -c -a emacs"
    ALTERNATE_EDITOR=""
   #+end_src
7. Add a keybinding for launching emacs client in =keybinds.conf=
   #+begin_src bash
