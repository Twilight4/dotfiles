...
* GUI Configuration
** Frame configuration
These settings only take effect on graphical frames — text-terminal frames ignore font =:family=/=:height= — so a single =config.org= serves both =emacsclient -c= (GUI) and =emacsclient -nw= (terminal) frames from the same =--daemon=. The font mirrors kitty's =JetBrainsMonoNLNerdFont= at 14pt so GUI text matches the kitty session. Animated cursor and window borders are provided by [[*Holo-layer][Holo-layer]], which supersedes the =window-divider-mode= / =vertical-border= approach used on the terminal.

#+begin_src emacs-lisp
;; Font — mirror kitty's "JetBrainsMonoNL Nerd Font" @ 14pt.
...
(set-face-attribute 'default nil
                    ...
                    :height 140)
(add-to-list 'default-frame-alist '(font . "JetBrainsMonoNL Nerd Font-14"))
...
** Centaur-tabs
[[https://github.com/ema2159/centaur-tabs][Centaur Tabs]] is an Emacs package that enhances the tab bar functionality, providing a more visually appealing way to manage multiple op en buffers. No point if using Emacs TUI.
...
(global-set-key (kbd "M-f") 'centaur-tabs-forward-tab)
(global-set-key (kbd "M-b") 'centaur-tabs-backward-tab)
(global-set-key (kbd "C-M-f") 'centaur-tabs-forward-group)
(global-set-key (kbd "C-M-b") 'centaur-tabs-backward-group)
#+end_src
...
(with-eval-after-load 'winner
  ...
  (define-key winner-mode-map (kbd "C-x w") 'winner-undo)
  (define-key winner-mode-map (kbd "C-x W") 'winner-redo))
...
(fringe-mode -1)                                  ; Disable the narrow areas on the sides of the window
...
(use-package cape
  :ensure t
  :commands (cape-dabbrev cape-file cape-elisp-block cape-dict)
  :init
...
(global-set-key (kbd "M-p") 'tl/jinx-auto-correct-save)
...
(with-eval-after-load 'treemacs
  ...
  (define-key treemacs-mode-map (kbd "j") 'treemacs-next-neighbour)
  (define-key treemacs-mode-map (kbd "k") 'treemacs-previous-neighbour)
...
;; Adding treemacs tui icons
(with-eval-after-load 'treemacs
  (setq treemacs-icon-fallback-text (propertize "󰈔 " 'face '(:foreground "#aaaaaa")))

  (treemacs-modify-theme "Default"
    :config
    (progn
      ;; directories
      (treemacs-create-icon :icon (propertize "󰝰 " 'face '(:foreground "#5599ff")) :fallback (propertize "󰝰 " 'face '(:foreground "#5599ff")) :extensions (dir-open))
      (treemacs-create-icon :icon (propertize "󰉋 " 'face '(:foreground "#5599ff")) :fallback (propertize "󰉋 " 'face '(:foreground "#5599ff")) :extensions (dir-closed))
      ;; root
      (treemacs-create-icon :icon (propertize "󱉭 " 'face '(:foreground "#ff9944")) :fallback (propertize "󱉭 " 'face '(:foreground "#ff9944")) :extensions (root-open))
      (treemacs-create-icon :icon (propertize "󱉭 " 'face '(:foreground "#ff9944")) :fallback (propertize "󱉭 " 'face '(:foreground "#ff9944")) :extensions (root-closed))
      ;; fallback
      (treemacs-create-icon :icon (propertize "󰈔 " 'face '(:foreground "#aaaaaa")) :fallback (propertize "󰈔 " 'face '(:foreground "#aaaaaa")) :extensions (fallback))

      ;; shell / scripts
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#89e051")) :fallback (propertize " " 'face '(:foreground "#89e051")) :extensions ("sh" "bash" "zsh" "fish" "bashrc" "zshrc" "zshenv" "zprofile" ".bashrc" ".zshrc" ".zshenv"))
      ;; images
      (treemacs-create-icon :icon (propertize "󰈟 " 'face '(:foreground "#a8d8ea")) :fallback (propertize "󰈟 " 'face '(:foreground "#a8d8ea")) :extensions ("png" "jpg" "jpeg" "gif" "bmp" "svg" "webp" "ico" "tiff"))
      ;; video
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#fd971f")) :fallback (propertize " " 'face '(:foreground "#fd971f")) :extensions ("mp4" "mkv" "avi" "mov" "webm" "flv" "wmv"))
      ;; audio
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#bc5fd3")) :fallback (propertize " " 'face '(:foreground "#bc5fd3")) :extensions ("mp3" "wav" "ogg" "flac" "aac" "m4a"))
      ;; archives
      (treemacs-create-icon :icon (propertize "󰛫 " 'face '(:foreground "#e8b86d")) :fallback (propertize "󰛫 " 'face '(:foreground "#e8b86d")) :extensions ("zip" "tar" "gz" "bz2" "xz" "7z" "rar" "zst"))
      ;; pdf / docs
      (treemacs-create-icon :icon (propertize "󰈦 " 'face '(:foreground "#ff5555")) :fallback (propertize "󰈦 " 'face '(:foreground "#ff5555")) :extensions ("pdf"))
      (treemacs-create-icon :icon (propertize "󰈬 " 'face '(:foreground "#6699ff")) :fallback (propertize "󰈬 " 'face '(:foreground "#6699ff")) :extensions ("doc" "docx" "odt"))
      (treemacs-create-icon :icon (propertize "󰈛 " 'face '(:foreground "#27ae60")) :fallback (propertize "󰈛 " 'face '(:foreground "#27ae60")) :extensions ("xls" "xlsx" "ods" "csv"))
      ;; markdown / text / org
      (treemacs-create-icon :icon (propertize "󰍔 " 'face '(:foreground "#ededed")) :fallback (propertize "󰍔 " 'face '(:foreground "#ededed")) :extensions ("md"))
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#77aa99")) :fallback (propertize " " 'face '(:foreground "#77aa99")) :extensions ("org"))
      (treemacs-create-icon :icon (propertize "󰈙 " 'face '(:foreground "#ededed")) :fallback (propertize "󰈙 " 'face '(:foreground "#ededed")) :extensions ("txt"))
      ;; web
      (treemacs-create-icon :icon (propertize "󰌝 " 'face '(:foreground "#e44d26")) :fallback (propertize "󰌝 " 'face '(:foreground "#e44d26")) :extensions ("html" "htm"))
      (treemacs-create-icon :icon (propertize "󰌜 " 'face '(:foreground "#264de4")) :fallback (propertize "󰌜 " 'face '(:foreground "#264de4")) :extensions ("css" "scss" "sass" "less"))
      ;; js / ts
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#cbcb41")) :fallback (propertize " " 'face '(:foreground "#cbcb41")) :extensions ("js" "mjs" "cjs" "json" "jsonc"))
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#519aba")) :fallback (propertize " " 'face '(:foreground "#519aba")) :extensions ("ts" "tsx"))
      (treemacs-create-icon :icon (propertize "󰜈 " 'face '(:foreground "#61dafb")) :fallback (propertize "󰜈 " 'face '(:foreground "#61dafb")) :extensions ("jsx"))
      ;; python
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#ffbc03")) :fallback (propertize " " 'face '(:foreground "#ffbc03")) :extensions ("py" "pyc" "pyd" "pyi"))
      ;; rust
      (treemacs-create-icon :icon (propertize "󱘗 " 'face '(:foreground "#dea584")) :fallback (propertize "󱘗 " 'face '(:foreground "#dea584")) :extensions ("rs"))
	  ;; php
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#a074c4")) :fallback (propertize " " 'face '(:foreground "#a074c4")) :extensions ("php"))
      ;; go
      (treemacs-create-icon :icon (propertize "󰟓 " 'face '(:foreground "#00add8")) :fallback (propertize "󰟓 " 'face '(:foreground "#00add8")) :extensions ("go"))
      ;; c / c++
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#5c9bd6")) :fallback (propertize " " 'face '(:foreground "#5c9bd6")) :extensions ("c" "h"))
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#519aba")) :fallback (propertize " " 'face '(:foreground "#519aba")) :extensions ("cpp" "cc" "cxx" "hpp"))
      ;; java
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#b07219")) :fallback (propertize " " 'face '(:foreground "#b07219")) :extensions ("java" "jar" "class"))
      ;; lua
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#51a0cf")) :fallback (propertize " " 'face '(:foreground "#51a0cf")) :extensions ("lua"))
      ;; config / data
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#6d8086")) :fallback (propertize " " 'face '(:foreground "#6d8086")) :extensions ("json" "jsonc"))
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#6d8086")) :fallback (propertize " " 'face '(:foreground "#6d8086")) :extensions ("db"))
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#aaaaaa")) :fallback (propertize " " 'face '(:foreground "#aaaaaa")) :extensions ("yaml" "yml"))
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#9c4221")) :fallback (propertize " " 'face '(:foreground "#9c4221")) :extensions ("toml"))
      (treemacs-create-icon :icon (propertize "󰗀 " 'face '(:foreground "#fd971f")) :fallback (propertize "󰗀 " 'face '(:foreground "#fd971f")) :extensions ("xml"))
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#aaaaaa")) :fallback (propertize " " 'face '(:foreground "#aaaaaa")) :extensions ("ini" "cfg" "conf" "config"))
      ;; git
      (treemacs-create-icon :icon (propertize "󰊢 " 'face '(:foreground "#f54d27")) :fallback (propertize "󰊢 " 'face '(:foreground "#f54d27")) :extensions ("gitignore" "gitconfig" "gitmodules" "gitattributes"))
      ;; docker
      (treemacs-create-icon :icon (propertize "󰡨 " 'face '(:foreground "#2496ed")) :fallback (propertize "󰡨 " 'face '(:foreground "#2496ed")) :extensions ("Dockerfile" "dockerignore" "docker-compose.yml" "docker-compose.yaml"))
      ;; makefile
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#6d8086")) :fallback (propertize " " 'face '(:foreground "#6d8086")) :extensions ("Makefile" "makefile" "mk"))
	  ;; elisp
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#8172be")) :fallback (propertize " " 'face '(:foreground "#8172be")) :extensions ("el" "elc"))
	  ;; log
      (treemacs-create-icon :icon (propertize "󰌱 " 'face '(:foreground "#aaaaaa")) :fallback (propertize "󰌱 " 'face '(:foreground "#aaaaaa")) :extensions ("log"))
	  ;; ttf
      (treemacs-create-icon :icon (propertize " " 'face '(:foreground "#aaaaaa")) :fallback (propertize " " 'face '(:foreground "#aaaaaa")) :extensions ("ttf"))
	  ;; pem / certs
      (treemacs-create-icon :icon (propertize "󰌋 " 'face '(:foreground "#cbcb41")) :fallback (propertize "󰌋 " 'face '(:foreground "#cbcb41")) :extensions ("pem" "crt" "cer" "key" "pub"))
      ;; license
      (treemacs-create-icon :icon (propertize "󰿃 " 'face '(:foreground "#cbcb41")) :fallback (propertize "󰿃 " 'face '(:foreground "#cbcb41")) :extensions ("LICENSE" "LICENCE")))))
...
(use-package ghostel
  ...
  (define-key ghostel-semi-char-mode-map (kbd "M-n") #'tl/ghostel-new)
...
(defun tl/ghostel-new ()
  ...
  (interactive)
  (let* ((current-prefix-arg '(4))
         (input (read-string "Buffer name: *ghostel-" '("*" . 0)))
