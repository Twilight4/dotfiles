...
(define-key org-mode-map (kbd "M-T") 'tl/org-insert-tag)
...
(general-create-definer tl/leader-keys
  ...
  (tl/leader-keys
    ...
    "A" '(org-gtd-engage :wk "Org GTD agenda")
    ...
    "p" '(org-gtd-process-inbox :wk "Process inbox") ;; process GTD inbox.org
...
*** Insert tag automation
Function to for =org-insert-tag= to insert a tag from a list.

#+begin_src emacs-lisp
(defun tl/org-insert-tag ()
  "Insert a tag from org-tag-alist."
  (interactive)
  (let* ((tag-alist (cl-remove-if (lambda (x) (or (eq x :startgroup) (eq x :endgroup))) (mapcar (lambda (x) (car x)) org-tag-alist)))
         (selected-tag (completing-read "Select tag: " tag-alist nil t)))
    (when (not (string= selected-tag ""))
      (org-set-tags-to (list selected-tag)))))
#+end_src
...
(setq org-gtd-capture-templates
  `(("t" "Todo" entry (file "~/documents/org-gtd/inbox.org")
    ...
    ("m" "Meeting" entry (file "~/documents/org-gtd/inbox.org")
         "* %<%I:%M %p> - %a :meetings:\n\n%?\n\n"
         :clock-in :clock-resume
         :empty-lines 1)

    ("c" "Metrics Capture")
     ("cw" "Weight" table-line (file+headline "~/documents/org-gtd/metrics.org" "Weight")
...
(use-package org-gtd
  ...
  :demand t
  :custom
  (org-gtd-directory "~/documents/org-gtd")      ;; TO CHANGE TO org/agenda ONCE CONVERTED ALL TASKS AND CORRECT org-gtd-capture-templates DIRECTORY
  (org-edna-use-inheritance t)
  (org-gtd-organize-hooks '(org-gtd-set-area-of-focus org-set-tags-command))
  :config
  (org-edna-mode)
  :bind
  (;("C-x p" . org-gtd-process-inbox)  ;; BOUND TO SPC p
   ;("C-x e" . org-gtd-engage)         ;; BOUND TO SPC E
   ("C-x i" . org-gtd-clarify-item)    ;; Mostly used when I'm in org-gtd-capture buffer to omit processint the task in inbox.org
   ;("C-x s" . org-gtd-clarify-switch-to-buffer) ;; BOUND TO SPC-B
   :map org-gtd-clarify-map
   ("C-x o" . org-gtd-organize)))
#+end_src
