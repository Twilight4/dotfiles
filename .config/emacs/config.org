...
;; Indent region
;;(global-set-key (kbd "C-M-\\") 'indent-region)   ; set by default
...
(general-define-key
 ...
 "C-C" '(avy-goto-char-2 :wk "Jump to a two-character char")
...
;; An attempt to always copy to the system clipboard using wl-copy
;;(defun copy-kill-ring-to-system-clipboard ()
;;  "Copy the contents of the last kill to the system clipboard using wl-copy."
;;  (interactive)
;;  (let ((text (substring-no-properties (car kill-ring))))
;;    (if (not (string= text ""))
;;        (progn
;;          (shell-command-on-region (point) (point) "wl-copy" nil nil nil)
;;          (message "Copied to system clipboard: %s" text))
;;      (message "Kill ring is empty"))))

;;(global-set-key (kbd "M-w") 'copy-kill-ring-to-system-clipboard)
#+end_src
