...
(defun tl/setup-elfeed-keys ()
  "Set up keybindings for Elfeed buffer."
  (define-key elfeed-search-mode-map (kbd "q") '+rss/quit)
  (define-key elfeed-search-mode-map (kbd "e") 'elfeed-update)
  ...
  (define-key elfeed-search-mode-map (kbd "S") 'elfeed-show-new-live-search)
  (define-key elfeed-search-mode-map (kbd "RET") 'elfeed-search-show-entry)
  (define-key elfeed-search-mode-map (kbd "P") 'elfeed-show-pdf)
  (define-key elfeed-search-mode-map (kbd "=") 'elfeed-search-tag-all)
  (define-key elfeed-search-mode-map (kbd "-") 'elfeed-search-untag-all)
  (define-key elfeed-search-mode-map (kbd "+") 'elfeed-show-tag)
  (define-key elfeed-search-mode-map (kbd "_") 'elfeed-show-untag)
  ...
  (define-key elfeed-search-mode-map (kbd "y") 'elfeed-search-yank)
  (define-key elfeed-search-mode-map (kbd "Y") 'elfeed-show-yank)
  (define-key elfeed-search-mode-map (kbd "o") 'ace-link-elfeed)
  (define-key elfeed-search-mode-map (kbd "a") 'org-ref-elfeed-add)
  (define-key elfeed-search-mode-map (kbd "n") 'elfeed-show-next)
  (define-key elfeed-search-mode-map (kbd "p") 'elfeed-show-prev)
  (define-key elfeed-search-mode-map (kbd "R") 'tl/elfeed-update-and-notify)
  (define-key elfeed-search-mode-map (kbd "D") 'tl/elfeed-download-video))

(add-hook 'elfeed-search-mode-hook #'tl/setup-elfeed-keys)
#+end_src
...
(setq bibtex-completion-bibliography '("~/documents/org/bibliography/references.bib"
					 "~/documents/org/bibliography/dei.bib"
					 "~/documents/org/bibliography/master.bib"
					 "~/documents/org/bibliography/archive.bib")
	bibtex-completion-library-path '("~/documents/org/bibliography/bibtex-pdfs/")
	bibtex-completion-notes-path "~/documents/org/bibliography/notes/"
...
*** Citar
Emacs package to quickly find and act on bibliographic references, and edit org, markdown, 
and latex academic documents. 

#+begin_src emacs-lisp
(use-package citar
  :no-require
  :custom
  (org-cite-global-bibliography '("~/documents/org/bibliography/references.bib"))
...
;; Use citar-org-roam to plugin to org-roam-bibtex
...
(setq bibtex-completion-bibliography
      '("~/documents/org/bibliography/bibtex-file-1.bib"
        "~/documents/org/bibliography/bibtex-file-2.bib"))

;; Basic configuration
;; PDF files
(setq bibtex-completion-library-path '("~/documents/org/bibliography/pdfs"))
(setq bibtex-completion-pdf-field "File")

;; Notes
(setq bibtex-completion-notes-path "~/documents/org/bibliography/notes.org")
#+end_src
...
(use-package org-gtd
  ...
  (org-gtd-organize-hooks '(org-gtd-set-area-of-focus org-set-tags-command org-set-effort org-schedule org-deadline))
  :config
  (org-edna-mode)
  :bind
