...
(global-set-key (kbd "C-M-i") 'gptel--suffix-rewrite)
(global-set-key (kbd "C-M-e") 'gptel--rewrite-directive-menu)
...
(use-package general
  ...
  (tl/leader-keys
    ...
    "v" '(tl/agent-shell-omp-start :wk "Start OMP agent-shell")
    ...
    "n" '(ghostel-next-prompt :wk "Ghostel next prompt")
    "p" '(ghostel-previous-prompt :wk "Ghostel previous prompt")
  ...
  (tl/leader-keys
    ...
    "C-t v" '(global-hl-line-mode :wk "Toggle line highlight globally")
...
(global-visual-line-mode 1)                       ; Display truncated lines
...
Do not save the =org-refile-last-stored= and =org-capture-last-stored= buffers as bookmarks upon emacs start.

#+begin_src emacs-lisp
(defadvice org-refile (around no-bookmark-refile act)
  (let ((bmkp-last-save-location-function)) ;Bind to nil to make this a no-op
    ad-do-it))

 (defadvice org-capture-place-template (around no-bookmark-capture act)
   (let ((bmkp-last-save-location-function))
       ad-do-it))
#+end_src
...
(use-package gptel
  ...
  (defun tl/gptel-suffix-send (args)
    "Call `gptel--suffix-send' with ARGS after loading `gptel-transient'."
    (unless (require 'gptel-transient nil t)
      (user-error "gptel-transient is not available"))
    (gptel--suffix-send args))
  ;; Global gptel actions, usable anywhere in Emacs.
...
(use-package visual-fill-column
  :ensure t
  :hook ((org-mode text-mode conf-mode Info-mode sh-mode-hook terraform-mode) . tl/visual-fill-column))
...
(use-package org-modern
  ...
  (setq org-modern-star '("◉" "○" "◈" "◇" "◆" "●")
        ...
        '(("TODO" :inverse-video t :inherit org-todo)
          ...
          ("CNCL" :inverse-video t :inherit +org-todo-onhold))
        org-modern-footnote
