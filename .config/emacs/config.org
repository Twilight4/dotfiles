...
- [[#editing-features][Editing Features]]
  ...
  - [[#select-until-beginningend-of-buffer][Select until beginning/end of buffer]]
...
- [[#other-packages][Other Packages]]
  ...
  - [[#desktop-override-stale-locks][Desktop-override-stale-locks]]
- [[#org-mode][Org Mode]]
...
#+begin_src emacs-lisp
;; Global Keybindings
;; Optionally available keybinds: M-r M-i C-v C-z C-t
...
(global-set-key (kbd "M-v") 'split-window-below)
...
;;(global-set-key (kbd "C-key") 'newline)
;; Redo
...
;;(global-set-key (kbd "C-S-e") 'org-end-of-line)  ;; correct it to have selection like C-S-a
;; Deleting keys
...
(global-set-key (kbd "C-.") 'kill-sentence)
;; Zooming keys
...
(global-set-key (kbd "M-O") 'dashboard-open)  ;; Wanted to open dashboard buffer in a new centaur-tab but didn't manage to do so
...
(global-set-key (kbd "m") 'surround-with-bold)
(global-set-key (kbd ",") 'surround-with-code)
(global-set-key (kbd ".") 'surround-with-green)
(global-set-key (kbd "/") 'surround-with-underline)
(global-set-key (kbd "'") 'surround-with-quote)
(global-set-key (kbd "\"") 'surround-with-double-quote)
...
(eval-after-load 'hl-todo
  '(progn
     (define-key hl-todo-mode-map (kbd "M-A") #'hl-todo-previous)
     (define-key hl-todo-mode-map (kbd "M-E") #'hl-todo-next)
     (define-key hl-todo-mode-map (kbd "M-I") #'hl-todo-insert)))
(define-key org-mode-map (kbd "M-t") 'org-todo)
(define-key org-mode-map (kbd "M-T") 'org-set-tags-command)
...
(general-create-definer tl/leader-keys
  ...
  (tl/leader-keys
    ...
    "d d" '(delete-file :wk "Delete file")
    "d r" '(rename-file :wk "Rename file")
...
(general-define-key
 :prefix "C-;"
 ;; Avy jumping
 "c" '(avy-goto-char :wk "Jump to a character")
 "C" '(avy-goto-char-2 :wk "Jump to a two-character char")
 "e" '(avy-goto-char-in-line :wk "Jump to a character in line")
 "a" '(avy-goto-subword-1 :wk "Jump to a subword")
 "l" '(avy-goto-line-below :wk "Jump to line below")
 "L" '(avy-goto-line-above :wk "Jump to line above")
 "o" '(avy-open-line :wk "Jump to line and insert new one above")
 "w" '(avy-goto-word-1-below :wk "Jump to a word below")
 "W" '(avy-goto-word-1-above :wk "Jump to a word above")
 "h" '(avy-org-goto-heading-timer :wk "Jump to org heading")
 ;; Avy yanking/killing/moving
 "k" '(avy-kill-region :wk "Kill region")
 "K" '(avy-kill-whole-line :wk "Kill whole line")
 "s" '(avy-kill-ring-save-region :wk "Save region")
 "S" '(avy-kill-ring-save-whole-line :wk "Save whole line")
 "y" '(avy-copy-region :wk "Copy region")
 "Y" '(avy-copy-line :wk "Copy line")
 "m" '(avy-move-region :wk "Move region")
 "M" '(avy-move-line :wk "Move line")
 ;; Avy navigation
 "n" '(avy-next :wk "Next occurance")
 "p" '(avy-prev :wk "Previous occurance")
 "r" '(avy-resume :wk "Resume last Avy action")
 "/" '(avy-pop-mark :wk "Jump to previous Avy location")
 "i" '(avy-isearch :wk "iSearch with Avy")
 "t" '(avy-transpose-lines-in-region :wk "Transpose lines in region"))
)
#+end_src
...
#+begin_src emacs-lisp
;; This code checks if the cursor is on a link or a checkbox and acts accordingly. If not in an Org mode buffer, it will call the default behavior for RET using call-interactively org-return.
(defun custom-return ()
  (interactive)
...
** Select until beginning/end of buffer
...
** Undo-tree
I'd like to also have the =redo= functionality. The default key are:
- =C-/= - undo
- =M-/= - redo
- =C-x u= - undo-tree-visualise
- =C-p/n= - navigate through the =undo-tree-visualise=
- =q= - quit
...
(setq desktop-restore-eager t)                    ; Restore all buffers eagerly (immediatelly), this can impact startup performance
...
(setq org-agenda-files
      '("~/documents/org/agenda/inbox.org"   ;; REMOVE IT CUZ I USE org-gtd-tasks.org instead
        "~/documents/org/agenda/workflow.org")) ;; DO NOT TOUCH IT
        ;"~/documents/org/agenda/habits.org"  ;; MOVE TO inbox.org instead
        ;"~/documents/org/agenda/goals.org"   ;; MOVE TO ORG-ROAM NOTE AND CREATE ORG-ROAM-CAPTURE TEMPLATE
        ;"~/documents/org/agenda/diet.org"))  ;; MOVE TO ORG-ROAM NOTE AND CREATE ORG-ROAM-CAPTURE TEMPLATE
...
;; Function to open Org Agenda frame
...
(setq org-gtd-capture-templates
  `(("t" "Todo" entry (file "~/documents/org-gtd/inbox.org")
         "* TODO %?\n  %U\n\n  %i" :empty-lines 1 :kill-buffer t)

    ("l" "Todo with link" entry (file "~/documents/org-gtd/inbox.org")
         "* TODO %?\n  %U\n  %i\n %a" :empty-lines 1 :kill-buffer t)

    ("a" "Appointment" entry (file "~/documents/org-gtd/inbox.org")
    ...
    ("m" "Metrics Capture")
     ("cw" "Weight" table-line (file+headline "~/documents/org-gtd/diet.org" "Weight")
         "| %U | %^{Weight} | %^{Notes} |" :kill-buffer t)
     ("cg" "Goals" table-line (file+headline "~/documents/org-gtd/goals.org" "Goals")
...
(use-package org-gtd
  ...
  ;(org-gtd-directory "~/documents/org/agenda")  ;; Once converted and organized the tasks with ORG-GTD ID, correct the org-gtd-capture-templates DIRECTORY
  (org-gtd-areas-of-focus '("Home" "Health" "Family" "Career"))
  (org-edna-use-inheritance t)
  (org-gtd-organize-hooks '(org-gtd-set-area-of-focus org-set-tags-command org-set-effort))
  :config
  (org-edna-mode)
  :bind
  (("C-x i" . org-gtd-clarify-item)    ;; Mostly used when I'm in org-gtd-capture buffer to omit processing the task from inbox.org
   ("C-c a" . org-gtd-clarify-agenda-item) ;; Used only in agenda-view
   ("C-c A" . org-gtd-area-of-focus-set-on-agenda-item) ;; Used only in agenda-view
