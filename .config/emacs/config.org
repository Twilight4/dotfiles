...
;; This code checks if in normal mode, check the cursor is on a link or a checkbox, and act accordingly. If not, it will insert a checkbox. If not in normal mode or if not in an Org mode buffer, it will call the default behavior for RET using call-interactively org-return.
(defun custom-return ()
  (interactive)
...
(eval-after-load 'evil
  '(define-key evil-normal-state-map (kbd "S-<return>") 'org-toggle-item))  ;; Key binding to toggle-item for evil normal mode
...
;; Using refiling for org-roam-dailiies
...
One interesting use for daily files is to keep a log of tasks that were completed on that particular day.
