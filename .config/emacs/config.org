...
;; pgtk GUI Emacs forwards every kill to the Wayland clipboard via
;; `interprogram-cut-function' (= `gui-select-text'), so C-u/C-o/C-w/M-d leak.
;; Disable it so kills stay in the kill-ring; explicit `tl/*-to-clipboard'
;; wrappers (which call wl-copy directly) remain the only path to the clipboard.
