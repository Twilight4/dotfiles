...
(eval-after-load 'hl-todo
  '(progn
     (define-key hl-todo-mode-map (kbd "C-S-j") #'hl-todo-previous)
     (define-key hl-todo-mode-map (kbd "C-S-k") #'hl-todo-next)
...
(setq org-tag-alist
  '((:startgroup)
     ...
     ("@Watch" . ?w)
     ("@Purchase" . ?p)
     ;("@Project" . ?p)    ; conflicts with org-gtd
     ;("@Read" . ?r)       ; unnecessary
     ("@Trivial" . ?t)))
#+end_src
...
(setq org-gtd-capture-templates
  `(("t" "Todo" entry (file "~/documents/org/agenda/inbox.org")
    ...
    ("m" "Metrics Capture")
     ("cw" "Weight" table-line (file+headline "~/documents/org/agenda/diet.org" "Weight")
         "| %U | %^{Weight} | %^{Notes} |" :kill-buffer t)
     ("cg" "Goals" table-line (file+headline "~/documents/org/agenda/goals.org" "Goals")
