...
(defun tl/forward-entire-word (&optional arg)
  "Move point forward by ARG symbols (default 1).
Unlike `forward-word`, this does not stop at punctuation like -, _, or ."
  (interactive "^p")
  (let ((arg (or arg 1)))
    (forward-symbol arg)))

(defun tl/backward-entire-word (&optional arg)
  "Move point backward by ARG symbols (default 1).
This is the backward equivalent of `forward-symbol-word`."
  (interactive "^p")
  (let ((arg (or arg 1)))
    (forward-symbol (- arg))))
#+end_src
