...
(use-package gptel
  ...
  (keymap-global-set "C-M-d" "M-g e m i RET")              ;; respond in place
  ...
  ;; Response options
  ;(global-set-key (kbd "C-x ") 'gptel--regenerate)
  ;(global-set-key (kbd "C-x ") 'gptel--next-variant)
  ;(global-set-key (kbd "C-x ") 'gptel--previous-variant)
)
#+end_src
...
(use-package god-mode
  ...
  (define-key god-local-mode-map (kbd "C-S-v") #'surround-underline)
  ;; Same keybinds as without god-mode (S-key instead of M-key)
  ;(define-key god-local-mode-map (kbd "S-RET") #'gptel-send)
