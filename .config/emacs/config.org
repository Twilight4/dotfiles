...
(general-define-key
 ...
 "C-o" '(tl/open-images-external-from-org :wk "Narrow to block")   ; Check if doesn't conflict with org-open-at-point
 "C-h" '(widen :wk "Widen")
...
#+begin_src emacs-lisp
;; Open images in external editor such as nsxiv
(defun tl/open-image-external-from-org ()
  "Open the image at point in an external viewer like nsxiv."
  (interactive)
  (let ((file (org-element-property :path (org-element-context))))
    (start-process "nsxiv" nil "nsxiv" file)))
#+end_src
