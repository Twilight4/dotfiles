...
- [[#editing-features][Editing Features]]
  ...
  - [[#smart-forwardbackward-word][Smart forward/backward word]]
...
(global-set-key (kbd "C-M-f") 'centaur-tabs-forward-tab)
(global-set-key (kbd "C-M-b") 'centaur-tabs-backward-tab)
(global-set-key (kbd "C-M-p") 'centaur-tabs-forward-group)
(global-set-key (kbd "C-M-n") 'centaur-tabs-backward-group)
#+end_src
...
;; Forward/backward entire words
(global-set-key (kbd "M-f") 'tl/forward-entire-word)
(global-set-key (kbd "M-b") 'tl/backward-entire-word)
;; Spell-checker
...
** Smart forward/backward word
Move the cursor through entire strings.

#+begin_src emacs-lisp
(defun tl/forward-entire-word (&optional arg)
  "Move point forward by ARG non-whitespace chunks.
A chunk is any sequence of non-whitespace characters, including
punctuation like .,,-,_:/."
  (interactive "^p")
  (let ((arg (or arg 1)))
    (dotimes (_ arg)
      ...
      (skip-syntax-forward " ")
      ;; Skip everything until next whitespace
      (skip-syntax-forward "^ "))))

(defun tl/backward-entire-word (&optional arg)
  "Move point backward by ARG non-whitespace chunks.
A chunk is any sequence of non-whitespace characters."
  (interactive "^p")
  (let ((arg (or arg 1)))
    (dotimes (_ arg)
      ;; Skip whitespace first
      (skip-syntax-backward " ")
      ;; Skip everything until previous whitespace
      (skip-syntax-backward "^ "))))
#+end_src
...
(defun tl/agent-shell-omp-start ()
  ...
  (interactive)
  (require 'agent-shell)
  (let ((buffer (agent-shell-start :config (tl/agent-shell-omp-make-config))))
