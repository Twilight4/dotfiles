...
- [[#editing-features][Editing Features]]
  ...
  - [[#close-frame-properly][Close frame properly]]
...
;(global-set-key (kbd "C-t") 'zap-to-char)
;(global-set-key (kbd "M-a") 'tl/previous-sentence)
;(global-set-key (kbd "M-e") 'tl/next-sentence)
...
;(global-set-key (kbd "M-u") 'backward-kill-sentence)
;(global-set-key (kbd "C-M-o") 'tl/kill-sentence)
;(global-set-key (kbd "C-M-u") 'tl/backward-kill-sentence)
...
(use-package general
  ...
  (tl/leader-keys
    ...
    "C-q" '(delete-frames-until-emacs-closes :wk "Exit Emacs")
    "O" '(tl/buffer-org-new :wk "New empty org buffer")
    "g" '(dashboard-open :wk "Open emacs dashboard")
...
(general-define-key
 ...
 "," '(copy-surrounded-by-equals :wk "Copy org equals command")
...
;; Hl-line-mode
(set-face-background 'hl-line "#141C23")  ;; Set to a dark shade of black
...
#+begin_src emacs-lisp
(defun delete-frames-until-emacs-closes ()
  "Delete the current frame repeatedly until Emacs closes."
  (interactive)
...
GPTel directives, find more prompts [[https://github.com/f/awesome-chatgpt-prompts][here]] and [[https://github.com/danielmiessler/fabric/tree/main/patterns][here]]. Check the find prompt script [[https://github.com/f/awesome-chatgpt-prompts/blob/main/scripts/find-prompt][here]].
#+begin_src emacs-lisp
...
;; Prompt from
(global-set-key (kbd "C-M-a") 'gptel-send-with-options)  ;; minibuffer
(keymap-global-set "C-M-d" "M-g ESC RET i RET")          ;; delete in place (like delete on C-d)

;; Response to
;; Option 'b' in 'gptel-menu' can't be set so u need to do: "M-g C-M-RET b"
(keymap-global-set "C-M-q" "M-g ESC RET e RET")      ;; to echo area (I preferably use gptel command)
(keymap-global-set "C-M-v" "M-g ESC RET g RET RET")  ;; send to gpt session (like gptel on C-M-c)

;; To/out of clipboard
(keymap-global-set "C-M-k" "M-g ESC RET y RET")      ;; prompt from kill ring
(keymap-global-set "C-M-l" "M-g ESC RET k RET")      ;; response to kill ring (use 'M-'' to copy it to system clipboard)
...
;; Other
(global-set-key (kbd "C-M-e") 'gptel-system-prompt)             ;; edit/choose prompt
...
(use-package god-mode
  ...
  (define-key god-local-mode-map (kbd "C-S-z") #'comment-dwim)
  (define-key god-local-mode-map (kbd "C-S-t") #'kmacro-start-macro-or-insert-counter)
  (define-key god-local-mode-map (kbd "C-S-i") #'org-cycle)
  (define-key god-local-mode-map (kbd "C-{") #'org-shifttab)
  ...
  (define-key god-local-mode-map (kbd "C-S-d") #'er/mark-paragraph)
  ...
  ;; Need to correct this to work only in visual mode
  ;;(define-key god-local-mode-map (kbd "m") #'insert-m)
  ;;(define-key god-local-mode-map (kbd ",") #'insert-comma)
  ;;(define-key god-local-mode-map (kbd ".") #'insert-period)
  ;;(define-key god-local-mode-map (kbd "/") #'insert-slash)
...
Automatically save the buffer to /tmp/ and load the last saved version if it exists."
  ...
  (let ((buffer-name "*org scratchpad*")
        (file-path "~/.cache/org-scratchpad.org"))
