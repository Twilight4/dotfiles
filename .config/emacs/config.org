...
(defconst tl/workspace-recipes
  '((agenda
    ...
    (work
     ...
     ("workspace"   . tl/ghostel-new-yazi)
     ("cheatsheets" . (lambda () (tl/ghostel-new-named "ghostel-cheatsheets")))
     ("notes"       . (lambda () (tl/ghostel-new-named "ghostel-notes")))
...
(use-package general
  ...
  (tl/leader-keys
    ...
    "RET" '(tl/ghostel-new :wk "Open new terminal")
...
(general-define-key
 ...
 ;; Org timer
 "C-x ;" '(org-timer-set-timer :wk "Set org timer")
 "C-x ." '(org-timer-pause-or-continue :wk "Pause or continue org timer")
 "C-x ," '(org-timer-stop :wk "Stop org timer")
 "C-x -" '(org-timer-item :wk "Insert org timer item")
 "C-x 0" '(org-timer-start :wk "Start org timer")
 "C-x '" '(org-timer-show-remaining-time :wk "Show remaining org timer time")
 ;; Org-Roam
...
(use-package multiple-cursors
  :bind
        (("C-x C-x C-c" . mc/edit-lines)
         ...
         ("C-x C-x C-r" . set-rectangular-region-anchor)
         ("C-x C-x r" . rectangle-mark-mode)))    ;; Backspace to delete, 't' to replace chars TODO
#+end_src
...
#+begin_src emacs-lisp
(defun tl/kill-region-to-clipboard ()
  "Kill the region if active."
  (interactive)
  (if (region-active-p)
      (progn
        (kill-ring-save (region-beginning) (region-end))
        (let ((process-connection-type nil))
          (start-process "wl-copy" "*wl-copy*" "wl-copy")
...
(defun tl/copy-to-system-clipboard ()
  (interactive)
  (if (region-active-p)
      (progn
        (kill-ring-save (region-beginning) (region-end))
...
(defun tl/paste-from-system-clipboard ()
...
(use-package ghostel
  ...
  (define-key ghostel-semi-char-mode-map (kbd "<tab>") #'god-local-mode)
  ...
  (define-key ghostel-semi-char-mode-map (kbd "M-s") #'tl/ghostel-new-yazi)
  (define-key ghostel-semi-char-mode-map (kbd "M-k") #'previous-multiframe-window)
  (define-key ghostel-semi-char-mode-map (kbd "M-j") #'next-multiframe-window)
...
(defun tl/ghostel-new-yazi ()
  "Always open a new ghostel buffer in the active directory and launch yazi instantly."
  (interactive)
  (let ((current-prefix-arg '(4))
        (default-directory default-directory))
    ;; 1. Spawn a clean, isolated ghostel buffer instance
    (call-interactively #'ghostel)
    (rename-buffer "ghostel-yazi" t)
    ;; 2. Give the underlying shell a tiny moment to wake up, then feed it yazi
    (run-with-timer 0.05 nil
                    (lambda (buf)
                      (when (buffer-live-p buf)
                        (with-current-buffer buf
                          ;; Inject "y" string followed by a newline carriage return
                          (ghostel-send-string "y\n"))))
                    (current-buffer))))
...
(global-set-key (kbd "M-s") 'tl/ghostel-new-yazi)
...
(use-package god-mode
  ...
  ;; Ctrl+w
  (define-key god-local-mode-map (kbd "w") #'tl/copy-to-system-clipboard)
  (define-key god-local-mode-map (kbd "C-w") #'backward-kill-word)
  (define-key god-local-mode-map (kbd "C-S-w") #'tl/copy-whole-line)

  ;; Ctrl+d
  (define-key god-local-mode-map (kbd "d") #'tl/kill-region-to-clipboard)
  ...
  (define-key god-local-mode-map (kbd "y") #'tl/paste-from-system-clipboard)
