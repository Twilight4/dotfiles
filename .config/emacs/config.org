...
(eval-after-load 'org-agenda
  '(progn
     ...
     (define-key org-agenda-mode-map (kbd "l") 'tl/org-agenda-day-view)
...
(use-package general
  ...
  (tl/leader-keys
    ...
    "C-k" '(previous-multiframe-window :wk "Switch window up")
...
(use-package ghostel
  ...
  (define-key ghostel-semi-char-mode-map (kbd "M-j") #'previous-multiframe-window)
  (define-key ghostel-semi-char-mode-map (kbd "M-k") #'next-multiframe-window)
...
(defvar tl/agent-shell-header-scale 0.7
  "Scale factor for the graphical Oh My Pi agent-shell header bar.
agent-shell renders its header as a fixed-size SVG image whose height
is 3× the frame char-height, so shrinking the `header-line' face has no
visible effect; the only lever is the header model's :font-height and
:font-size, which scale the whole bar (height, text, icon) uniformly.
1.0 = native size.")

(defun tl/agent-shell-scale-header-model (model)
  "Scale the graphical agent-shell header MODEL's font metrics.
See `tl/agent-shell-header-scale'.  Applied as :filter-return advice on
`agent-shell--make-header-model'; leaves non-graphical styles untouched."
  (when (and (boundp 'agent-shell-header-style)
    ...
    (let ((s tl/agent-shell-header-scale))
      (map-put! model :font-height
                (max 1 (round (* s (map-elt model :font-height)))))
      (map-put! model :font-size
                (* s (or (map-elt model :font-size)
                         (map-elt model :font-height))))))
  model)
