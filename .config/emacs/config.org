...
(use-package general
  ...
  (tl/leader-keys
    ...
    "a i" '(tl/org-agenda-i :wk "Agenda IPT workflow")
    "a S" '(tl/org-agenda-S :wk "Agenda Sabotage workflow")
...
(use-package flyspell
  ...
  :bind (:map flyspell-mode-map
              ;; Disabled ones don't work in emacs in terminal
			  ;; Bind flyspell-correct-word-before-point
              ("C-;" . nil)
              ("C-," . nil)
              ("C-." . nil)
              ("C-M-_" . flyspell-goto-next-error)
...
GPTel directives, find more prompts [[https://github.com/f/awesome-chatgpt-prompts][here]] and [[https://github.com/danielmiessler/fabric/tree/main/patterns][here]].
#+begin_src emacs-lisp
...
Automatically save the buffer to /tmp/ and load the last saved version if it exists."
  ...
  (let ((buffer-name "*org scratchpad*")
        (file-path "/tmp/org-scratchpad.org"))
    ...
    (add-hook 'after-save-hook
              (lambda ()
                (when (string= (buffer-name) buffer-name)
                  (let ((file-path "/tmp/org-scratchpad.org"))
                    (save-buffer)
                    (message "Auto-saved *org scratchpad* to %s" file-path))))
              nil t)))

(defun tl/save-scratchpad ()
  "Save the *org scratchpad* buffer to /tmp/org-scratchpad.org."
  (let ((buffer-name "*org scratchpad*")
        (file-path "/tmp/org-scratchpad.org"))
...
(setq org-directory "~/documents/org/agenda"
      ...
      org-todo-keywords
        '((sequence
           ...
           "NEXT(n)"
           "SOMEDAY(s)"
           "FINISHED(f)"   ; I keep these as active cuz I want to keep them in agenda view
           "CANC(c)"       ; I keep these as active cuz I want to keep them in agenda view
...
(setq org-tag-alist
  '((:startgroup)
     ...
     ;; Workflow tags
     ("@Pre_Engagement" . ?q)
     ("@Open_Source_Intelligence" . ?o)
	   ("@Weaponization" . ?A)
     ("@AD_Exploitation" . ?E)
     ("@AD_Internal_Enumeration" . ?n)
     ("@AD_Post_Exploitation" . ?P)
     ("@AD_Post_Domain_Compromise" . ?D)
     (:endgroup)
))
#+end_src
...
(defun tl/org-agenda-i ()
  "Run org agenda command 'w' with a specific agenda file."
  (interactive)
  (let ((org-agenda-files '("~/documents/org/agenda/ipt_workflow.org")))
    (org-agenda nil "i")))

(defun tl/org-agenda-S ()
  "Run org agenda command 'w' with a specific agenda file."
  (interactive)
  (let ((org-agenda-files '("~/documents/org/agenda/sabotage_workflow.org")))
    (org-agenda nil "S")))
        ...
        ((alltodo "" ((org-agenda-overriding-header "")
                      (org-super-agenda-groups
                       '((:name " Pre-Engagement"
                          :tag "@Pre_Engagement"
                          :order 1)
                         (:name "󰚌 AD Exploitation"
                          :tag "@AD_Exploitation"
                          :order 2)
                         (:name "󰈈 AD Internal Enumeration"
                          :tag "@AD_Internal_Enumeration"
                          :order 3)
                         (:name " AD Post Exploitation"
                          :tag "@AD_Post_Exploitation"
                          :order 4)
                         (:name " Post Domain Compromise"
                          :tag "@AD_Post_Domain_Compromise"
                          :order 5))))))))
        ...
        ((alltodo "" ((org-agenda-overriding-header "")
                      (org-super-agenda-groups
                       '((:name " Pre-Engagement"
                          :tag "@Pre_Engagement"
                          :order 1)
                         (:name "󰖟 Open-Source Intelligence"
                          :tag "@Open_Source_Intelligence"
                          :order 2)
                         (:name "󰄻 Weaponization"
                          :tag "@Weaponization"
                          :order 3)
                         (:name "󰚌 Initial Foothold"
                          :tag "@AD_Exploitation"
                          :order 4)
                         (:name " Host Post Exploitation"
                          :tag "@AD_Post_Exploitation"
                          :order 5)
...
(setq org-roam-capture-templates
  '(("d" "default" plain "%?"
    ...
    ("wi" "Internal Pentest" plain
     ...
     :if-new (file "~/documents/org/agenda/ipt_workflow.org")
     :unnarrowed t)
    ("we" "External Pentest" plain
     (file "~/documents/org/templates/workflows/ept-workflow.org")
     :if-new (file "~/documents/org/agenda/ept_workflow.org")
     :unnarrowed t)
    ("wr" "Red Team Engagement" entry
     (file "~/documents/org/templates/workflows/red-team-workflow.org")
     :if-new (file "~/documents/org/agenda/red-team_workflow.org")
     :unnarrowed t)
    ("wS" "Sabotage Delivery Service" entry
     (file "~/documents/org/templates/workflows/sabotage-workflow.org")
     :if-new (file "~/documents/org/agenda/sabotage_workflow.org")
     :unnarrowed t)
    ("ws" "Social Engineering Pentest" entry
     (file "~/documents/org/templates/workflows/se-workflow.org")
     :if-new (file "~/documents/org/agenda/se_workflow.org")
     :unnarrowed t)
    ("ww" "Wireless Pentest" entry
     (file "~/documents/org/templates/workflows/wireless-workflow.org")
     :if-new (file "~/documents/org/agenda/wireless_workflow.org")
     :unnarrowed t)
    ("l" "Logbook Entry")
	  ("le" "Log Quick Entry" plain
     (file "~/documents/org/templates/logbook/log-entry.org")
     :if-new (file+olp "~/documents/org/reports/${slug}_logbook.org" ("Technical Findings"))
     :unnarrowed t)
