...
(general-define-key
 ...
 "I" '(org-toggle-inline-images :wk "Display inline images")
 ;; Org-Roam
...
#+begin_src emacs-lisp
;; Function to toggle a checkbox bound to Return
(defun custom-return ()
  (interactive)
...
;; Funtion to not toggle a checkbox bound to C-m
(defun toggle-checkbox-return ()
  (interactive)
  (if (org-at-item-p) ; Check if at a checkbox item
      (org-toggle-checkbox)
...
;;            (local-set-key (kbd "C-m") 'toggle-checkbox-return)))
#+end_src
...
(defun open-dired-in-terminal ()
  "Open a Dired buffer in Emacs inside a terminal."
  (interactive)
  (let ((default-directory (if (derived-mode-p 'dired-mode)
                              (dired-get-file-for-visit)
                            default-directory)))
    (start-process "emacsclient" nil "emacsclient" "-t" "-e" (concat "(dired \"" default-directory "\")"))))
