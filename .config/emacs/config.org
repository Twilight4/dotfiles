...
(use-package general
  ...
  (tl/leader-keys
    ...
    "C-v" '(rectangle-mark-mode :wk "Rectange mark mode")
    ...
    "C-a" '(mark-whole-buffer :wk "Mark whole buffer")
	  ...
	  "C-m" '(tl/open-video-link-in-mpv :wk "Open video in mpv")
	  ;; Buffers
...
(general-define-key
 ...
 "l" '(mark-whole-line :wk "Mark whole line")
...
(defun mark-whole-line ()
  "Mark the whole line from start to end position, with the cursor at the end."
  (interactive)
  (let ((start (line-beginning-position))
        (end (line-end-position)))
    (goto-char end)                ; Move cursor to the end of the line
    (set-mark start)))             ; Set the mark at the beginning of the line
#+end_src
...
(use-package god-mode
  ...
  (define-key god-local-mode-map (kbd "C-u") #'org-backward-sentence)
  (define-key god-local-mode-map (kbd "C-k") #'org-forward-sentence)
  (define-key god-local-mode-map (kbd "C-S-u") #'org-backward-sentence)
  (define-key god-local-mode-map (kbd "C-S-k") #'org-forward-sentence)
  ...
  (define-key god-local-mode-map (kbd "C-d") #'forward-word)
  (define-key god-local-mode-map (kbd "C-h") #'backward-word)
  (define-key god-local-mode-map (kbd "C-S-d") #'forward-word)
  (define-key god-local-mode-map (kbd "C-S-h") #'backward-word)
  ...
  ;; Don't skip any buffers
  (setq god-exempt-major-modes nil)
  (setq god-exempt-predicates nil)

  ;; Don't enable in dired
