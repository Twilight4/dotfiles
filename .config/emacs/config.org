...
- [[#editing-features][Editing Features]]
  - [[#better-forwardbackward-word][Better forward/backward word]]
...
(global-set-key (kbd "C-f") 'tl/forward-word)
(global-set-key (kbd "C-b") 'tl/backward-word)
...
* Editing Features
** Better forward/backward word
In commands forward/backward word to not stop the point by characters like '-' and skip the entire words by default. Skip entire strings of non-space characters.

#+begin_src emacs-lisp
;; Custom forward-word function to skip entire non-space strings
(defun tl/forward-word (&optional arg)
  "Move forward ARG words, treating any non-space sequence as a single word."
  (interactive "p")
  (let ((arg (or arg 1)))
    (if (< arg 0)
        (my-backward-word (- arg))
      (dotimes (_ arg)
        (skip-syntax-forward "^ ") ; Skip all non-space characters
        (skip-syntax-forward " ")  ; Skip spaces to the next word
        ))))

;; Custom backward-word function to skip entire non-space strings
(defun tl/backward-word (&optional arg)
  "Move backward ARG words, treating any non-space sequence as a single word."
  (interactive "p")
  (let ((arg (or arg 1)))
    (if (< arg 0)
        (my-forward-word (- arg))
      (dotimes (_ arg)
        (skip-syntax-backward " ")  ; Skip spaces backward
        (skip-syntax-backward "^ ") ; Skip all non-space characters backward
        ))))

;; Custom forward-word function to skip entire non-space strings and mark the region (for god-mode)
(defun tl/forward-mark-word (&optional arg)
  "Move forward ARG words, treating any non-space sequence as a single word.
If called interactively, mark the region as you move forward, similar to C-S-f."
  (interactive "p")
  (let ((arg (or arg 1)))
    (if (< arg 0)
        (my-backward-word (- arg))
      (when (and (called-interactively-p 'any) (not (region-active-p)))
        (push-mark (point) t t)) ; Set the mark at the current position
      (dotimes (_ arg)
        (skip-syntax-forward "^ ") ; Skip all non-space characters
        (skip-syntax-forward " ")  ; Skip spaces to the next word
        )
      (when (called-interactively-p 'any)
        (activate-mark))))) ; Activate the mark to highlight the region

;; Custom backward-word function to skip entire non-space strings and mark the region (for god-mode)
(defun tl/backward-mark-word (&optional arg)
  "Move backward ARG words, treating any non-space sequence as a single word.
If called interactively, mark the region as you move backward, similar to C-S-b."
  (interactive "p")
  (let ((arg (or arg 1)))
    (if (< arg 0)
        (tl/forward-word (- arg)) ; Use the forward-word function for negative ARG
      (when (and (called-interactively-p 'any) (not (region-active-p)))
        (push-mark (point) t t)) ; Set the mark at the current position
      (dotimes (_ arg)
        (skip-syntax-backward " ")  ; Skip spaces to the previous word
        (skip-syntax-backward "^ ") ; Skip all non-space characters backward
        )
      (when (called-interactively-p 'any)
        (activate-mark))))) ; Activate the mark to highlight the region
...
(use-package multiple-cursors
  :bind ;; Mark one more occurrence
        (("C-x m" . mc/mark-next-like-this)
         ("C-x C-x C-A" . mc/mark-all-like-this)
...
(use-package god-mode
  ...
  (define-key god-local-mode-map (kbd "C-f") #'tl/forward-word)
  (define-key god-local-mode-map (kbd "C-b") #'tl/backward-word)
  (define-key god-local-mode-map (kbd "C-S-f") #'tl/forward-mark-word)
  (define-key god-local-mode-map (kbd "C-S-b") #'tl/backward-mark-word)
