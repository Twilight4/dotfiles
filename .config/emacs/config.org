...
(with-eval-after-load 'treemacs
  ...
  (define-key treemacs-mode-map (kbd "M-n") 'treemacs-quit)
...
(use-package god-mode
  ...
  (define-key god-local-mode-map (kbd "C-S-k") #'org-kill-line)
  (define-key god-local-mode-map (kbd "C-S-u") #'tl/org-kill-line-backward)
  ...
  (define-key god-local-mode-map (kbd "C-S-v") #'upcase-initials-region)
  (define-key god-local-mode-map (kbd "C->") #'tl/toggle-case-word-at-point)
  (define-key god-local-mode-map (kbd "C-<") #'tl/toggle-case-region)
  (define-key god-local-mode-map (kbd "C-S-l") #'mark-whole-line)
  ...
  (define-key god-local-mode-map (kbd "C-~") #'tl/toggle-case-char)
  ...
  ;(define-key god-local-mode-map (kbd "C-S-b") #'org-babel-mark-block)
...
(defun tl/toggle-case-char ()
  "Toggle the case of the character under the cursor without moving the point."
  (interactive)
  (let ((char (char-after)))
    (when char
      (let ((new-char (if (eq char (upcase char))
                          (downcase char)
                        (upcase char))))
        (save-excursion
          (delete-char 1)
...
(defun tl/toggle-case-word-at-point ()
  "Toggle the case of the word at the current point (upcase if downcased, downcase if upcased)."
  (interactive)
  (save-excursion
    (let* ((bounds (bounds-of-thing-at-point 'word))
           (word (buffer-substring-no-properties (car bounds) (cdr bounds)))
           (new-word (if (string= word (upcase word))
                         (downcase word)
                       (upcase word))))
      (when bounds
...
(defun tl/toggle-case-region ()
  "Toggle the case of the active region (upcase if downcased, downcase if upcased)."
  (interactive)
  (when (use-region-p)
    (let* ((bounds (cons (region-beginning) (region-end)))
           (text (buffer-substring-no-properties (car bounds) (cdr bounds)))
           (new-text (if (string= text (upcase text))
                         (downcase text)
                       (upcase text))))
      (delete-region (car bounds) (cdr bounds))
      (insert new-text))))
