...
(use-package general
  ...
  (tl/leader-keys
    ...
    "C-s C-v" '(describe-key-variable :wk "Describe key briefly")
...
(defvar tl/agent-shell-header-scale 0.5
  "Scale factor for the graphical Oh My Pi agent-shell header bar HEIGHT.
agent-shell renders its header as a fixed-size SVG image whose height is
3× the frame char-height, so shrinking the `header-line' face has no
effect; scaling the header model's :font-height (icon + layout) is the
lever.  The text (:font-size) is kept as large as the scaled bar allows,
not shrunk with it.  1.0 = native size.")

(defun tl/agent-shell-scale-header-model (model)
  "Scale the graphical agent-shell header MODEL's height, keeping text large.
Scales :font-height by `tl/agent-shell-header-scale' and clamps :font-size
to the scaled height, so the bar shortens without shrinking the text more
than the shorter bar requires.  Applied as :filter-return advice on
`agent-shell--make-header-model'; non-graphical styles are untouched."
  (when (and (boundp 'agent-shell-header-style)
             (eq agent-shell-header-style 'graphical)
             (numberp tl/agent-shell-header-scale)
             (/= tl/agent-shell-header-scale 1.0))
    (let* ((base-h (map-elt model :font-height))
           (new-h (max 1 (round (* tl/agent-shell-header-scale base-h))))
           (native-size (or (map-elt model :font-size) base-h)))
      (map-put! model :font-height new-h)
      (map-put! model :font-size (min new-h native-size))))
  model)
...
(advice-add 'agent-shell--make-header-model :filter-return
            #'tl/agent-shell-scale-header-model)
