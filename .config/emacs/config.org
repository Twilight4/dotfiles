...
(defun open-org-agenda-frame ()
  "Open a new frame with org-gtd-engage-grouped-by-context command."
  (interactive)
  (let ((agenda-frame (make-frame)))
    (select-frame-set-input-focus agenda-frame)
    (with-selected-frame agenda-frame
      (require 'org-gtd) ; Make sure org-gtd is loaded
      (org-gtd-engage-grouped-by-context)))) ; Use the new command
#+end_src
