...
(with-eval-after-load 'treemacs
  ...
  (define-key treemacs-mode-map (kbd "nf") 'treemacs-create-file)
  (define-key treemacs-mode-map (kbd "nd") 'treemacs-create-dir)
...
(defun tl/log-entry ()
  ...
  (let ((current-date-time (format-time-string "%Y-%m-%d %I:%M %p")))
    (end-of-line)
    (insert-file-contents "~/documents/org/templates/entry-snippets/log-entry.org")
    (next-line)
...
(defun tl/report-entry ()
  ...
  (let ((current-date-time (format-time-string "%Y-%m-%d %I:%M %p")))
    (end-of-line)
    (insert-file-contents "~/documents/org/templates/entry-snippets/report-entry.org")
    (next-line)
    (insert "#+DATE: " current-date-time "\n")
    (previous-line)
    (previous-line)
