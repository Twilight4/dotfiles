...
(defun tl/agent-shell-omp-start (&optional arg)
  "Start an interactive oh-my-pi agent shell and return its buffer.

Without a prefix ARG, the shell runs in the current buffer's
`default-directory' (resolved through project detection via
`agent-shell-cwd').  With a prefix ARG, prompt for a directory to
use as the shell's working directory -- handy for pointing OMP at a
project tree (e.g. a linux source dir with its own context files)
without first navigating there.  The chosen directory becomes the
omp subprocess `cwd' and the shell title."
  (interactive "P")
  (require 'agent-shell)
  (let* ((default-directory
          (if arg
              (file-name-as-directory
               (expand-file-name
                (read-directory-name "OMP working directory: ")))
            default-directory))
         (buffer (agent-shell-start :config (tl/agent-shell-omp-make-config))))
