...
(use-package highlight-indent-guides
  ...
  :init
  (setq highlight-indent-guides-method 'character) ;; 'column, 'bitmap, or 'character
  (setq highlight-indent-guides-character ?\│)      ;; vertical bar
  (setq highlight-indent-guides-auto-enabled t)
  :config
  ;; Function to enable highlight-indent-guides in programming or YAML/text-like modes
  (defun my/enable-highlight-indent-guides ()
	"Enable `highlight-indent-guides-mode` for prog-mode or yaml-mode and other text-like modes."
	(when (or (derived-mode-p 'prog-mode)
			  (derived-mode-p 'yaml-mode))
	  (highlight-indent-guides-mode 1)))

  ;; Set color dynamically when the mode starts
...
(defun tl/skip-invisible-forward (&rest _)
  (while (and (not (eobp))
              (get-char-property (point) 'invisible))
    (goto-char (next-single-char-property-change (point) 'invisible))))

(defun tl/skip-invisible-backward (&rest _)
  (while (and (not (bobp))
              (get-char-property (1- (point)) 'invisible))
    (goto-char (previous-single-char-property-change (point) 'invisible))))

(advice-add 'forward-char  :after #'tl/skip-invisible-forward)
(advice-add 'backward-char :after #'tl/skip-invisible-backward)
#+end_src
...
(use-package god-mode
  ...
  ;; F-keys
  (define-key god-local-mode-map (kbd "<f3>") #'kmacro-start-macro-or-insert-counter)
  (define-key god-local-mode-map (kbd "<f4>") #'kmacro-end-or-call-macro)
...
(use-package org-fancy-priorities
  ...
  (setq
    ...
    '((?A . 'all-the-icons-red)
      ...
      (?B . '(:foreground "#F08000")) ;; Adjusted for brighter yellow
...
'(("*" (bold :slant italic :weight black :foreground "#e6b450" ))
