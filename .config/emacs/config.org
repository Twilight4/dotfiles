...
(define-key calendar-mode-map "O" 'calendar-goto-date)
; Other
...
;; Agenda-mode - bind: org-save-all-org-buffers, org-timer-set-timer (and other timer commands - for agenda view), org-agenda-recenter, org-agenda-toggle-time-grid, org-agenda-goto-date, org-agenda-capture, tl/org-agenda-done (x), tl/org-agenda-mark-done-and-add-followup (X), tl/org-agenda-new (N)
(eval-after-load 'org-agenda
  '(progn
     ...
     (define-key org-agenda-mode-map (kbd "Y") 'org-agenda-month-view)
     (define-key org-agenda-mode-map (kbd "SPC") 'org-agenda-bulk-toggle)
     (define-key org-agenda-mode-map (kbd "m") 'org-agenda-bulk-toggle-all)
     (define-key org-agenda-mode-map (kbd ";") 'org-agenda-show-tags)
     (define-key org-agenda-mode-map (kbd "L") 'org-agenda-phases-of-moon)
     (define-key org-agenda-mode-map (kbd "G") 'org-agenda-holidays)
     (define-key org-agenda-mode-map (kbd "M") 'org-agenda-bulk-action)
     (define-key org-agenda-mode-map (kbd "Q") 'org-agenda-exit)
     ;(define-key org-agenda-mode-map (kbd "Q") 'org-agenda-show-scroll-down)
     ;(define-key org-agenda-mode-map (kbd "O") 'org-agenda-show-scroll-up)
...
(use-package general
  ...
  (tl/leader-keys
    ...
    "C-g" '(dashboard-open :wk "Open emacs dashboard")
    ...
    "C-h" '(org-table-insert-row :wk "Insert a new row above the current line")
    "k" '(org-table-kill-row :wk "Delete the current row or horizontal line")
    "u" '(org-table-delete-column :wk "Delete a column")
    ;"q" '(org-table-blank-field :wk "Blank field or active region")
	  ;; Window management
    "C-c" '(clone-indirect-buffer-other-window :wk "Clone indirect buffer in new window")
  ...
  (tl/leader-keys
    ...
    "a o" '(tl/org-agenda-o :wk "Agenda overview")
    "a t" '(tl/org-agenda-t :wk "Agenda today")
    "a e" '(tl/org-agenda-e :wk "Agenda by low efforts")
  ...
  (tl/leader-keys
    "h" '(:ignore t :wk "Help")
    "h h" '(help-for-help :wk "Get help")
    "h i" '(Info-goto-emacs-command-node :wk "Go to info mode in emacs MANUAL for command")
    "h I" '(info :wk "Enter info, the documentation browser")
    "h e" '(info-emacs-manual :wk "Display emacs manual in info mode")
    "h E" '(info-display-manual :wk "Display info buffer displaying MANUAL")
    "h m" '(describe-mode :wk "Display documentation of current modes")
...
(general-define-key
 ...
 "C-p" '(org-previous-visible-heading :wk "Org metaright")
 "C-n" '(org-next-visible-heading :wk "Org metaright")
 ;"C-." '(org-metaup :wk "Org shift up")
 ;"C-," '(org-metadown :wk "Org shift down")
...
(general-define-key
 ...
 ;"M-s" '(symbol-overlay-save-symbol :wk "Symbol save")      ;; taken by kitty
 ...
 "C-R" '(symbol-overlay-rename :wk "Symbol rename")
 ;; Expand-region
 "e" '(org-mark-element :wk "Mark element")       
...
;; Set background colors to the same as in kitty terminal background
(custom-set-faces
 '(default ((t (:background "#040305"))))
 '(line-number ((t (:background "#040305"))))
 '(mode-line ((t (:background "#16161e"))))
 '(mode-line-inactive ((t (:background "#16161e"))))
 ;; Set the text color in org-mode source code blocks
 '(org-block ((t (:foreground "#CDD6F4")))))

;; Hl-line-mode
(set-face-background 'hl-line "#16161e")  ;; Set to a dark shade of black
...
(keymap-global-set "M-i" #'jinx-correct)
(keymap-global-set "M-n" #'jinx-correct-nearest)
(keymap-global-set "M-w" #'jinx-correct-word)
;(keymap-global-set "M-a" #'jinx-correct-all)
;(keymap-global-set "M-w" #'jinx-correct-select)
#+end_src
...
(use-package gptel
  ...
  (global-set-key (kbd "C-x E") 'gptel--rewrite-directive-menu)
  ...
  ;; Other
  (global-set-key (kbd "C-x s") 'gptel-system-prompt)
  ...
  ;; Response options
  (global-set-key (kbd "C-x C-r") 'gptel--regenerate)
  (global-set-key (kbd "C-x C-f") 'gptel--next-variant)
...
(use-package god-mode
  ...
  (define-key god-local-mode-map (kbd "C-S-w") #'query-replace)
  ...
  (define-key god-local-mode-map (kbd "C-S-r") #'rectangle-mark-mode)    ;; Press 't' to replace chars
...
(global-set-key (kbd "C-x L") 'tl/log-entry)
...
(global-set-key (kbd "C-x l") 'tl/log-entry-small)
...
(global-set-key (kbd "C-x k") 'tl/report-entry)
#+end_src
...
(use-package org-gtd
  ...
  (;("C-x i" . org-gtd-clarify-item)  ;; Used in org-gtd-tasks.org buffer for quick changes
   ;("C-c a" . org-gtd-clarify-agenda-item)  ;; Used in org-gtd-agenda-view
   ;("C-c A" . org-gtd-area-of-focus-set-on-agenda-item)  ;; Used in org-gtd-agenda-view
