...
(use-package general
  ...
  (tl/leader-keys
    ...
    "C-d h" '(dired-hide-details-mode :wk "Toggle file details"))
  ...
  (when (and (display-graphic-p) (executable-find "wl-paste"))
    (let ((clip (shell-command-to-string "wl-paste")))
      (unless (or (string-empty-p clip)
        ...
        (unless (equal clip (current-kill 0 t))
          (kill-new clip))))))
...
(use-package peep-dired
  :after dired
  :config
  ;; j/k live in the *minor-mode* map so they steer the peep preview only while
  ;; peep-dired is ON; with it off they fall through to dired-mode-map's
  ;; next/previous-line (below) — no hook/advice needed.
...
(defun tl-dired-mode-setup ()
  ...
  ;; Display: hide perms/owner/size/date by default → icons + names only.
  ;; Toggle (peek at details) via `C-x C-d h'. Stock `(' unbound.
