...
(define-key global-map (kbd "M-g") 'gptel-menu)
...
(use-package gptel
  ...
  ;; Prompt options
  (global-set-key (kbd "C-M-a") 'gptel-send-with-options)  ;; minibuffer to echo area
  (keymap-global-set "C-M-k" "M-g m y RET")                ;; prompt from kill ring
  (keymap-global-set "C-M-d" "M-g e m i RET")              ;; respond in place (like delete on C-d)
  ;(keymap-global-set "C-M-q" "M-g m RET")                 ;; to echo area (I preferably use gptel command)
  ;(keymap-global-set "C-M-v" "M-g e g RET RET")           ;; send to gpt session
  ;(keymap-global-set "C-M-l" "M-g k RET")                 ;; response to kill ring (use 'M-'' to copy it to system clipboard)
