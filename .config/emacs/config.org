...
(with-eval-after-load 'winner
  (define-key winner-mode-map (kbd "C-c C-<left>") nil)
  (define-key winner-mode-map (kbd "C-c C-<right>") nil)
  (define-key winner-mode-map (kbd "C-x w") 'winner-undo)
  (define-key winner-mode-map (kbd "C-x W") 'winner-redo))
(global-set-key (kbd "C-x r r") 'consult-register)
;; Movement keys
...
;(global-set-key (kbd "C-t") 'treemacs-add-and-display-current-project-exclusively)
...
(use-package god-mode
  ...
  ;(define-key god-local-mode-map (kbd "t") #'treemacs-add-and-display-current-project-exclusively)
...
(defun tl/consult-new-org ()
  "Switch to the latest modified '*new org*' buffer, then run `consult-buffer`.
If the minibuffer is cancelled with C-g, kill the Emacsclient frame."
  (interactive)
  (let ((default-name "*new-org*"))
    ;; Find latest modified *new org* buffer
    (let ((latest-buffer
           (car (sort
                 (seq-filter (lambda (buf)
                               (string= (buffer-name buf) default-name))
                             (buffer-list))
                 (lambda (a b)
                   (time-less-p (buffer-local-value 'buffer-display-time b)
                                (buffer-local-value 'buffer-display-time a)))))))
      (when latest-buffer
        (switch-to-buffer latest-buffer)))
    ;; Wrap minibuffer call to handle C-g
    (condition-case err
        (minibuffer-with-setup-hook
            (lambda () (insert default-name))
          (call-interactively #'consult-buffer))
      (quit
       ;; If the user pressed C-g, close the current frame
       (when (and (fboundp 'server-delete-client)
