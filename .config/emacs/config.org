...
(setq org-roam-capture-templates
  '(("d" "default" plain "%?"
    ...
    ("l" "Logbook-1 Entry")
     ...
     :target (file+olp "~/documents/org/reports/logbook-1.org" ("Logbook"))
     :unnarrowed t)
...
;     :unnarrowed t)
    ("o" "Logbook-2 Entry")
	  ("on" "New Logbook" plain
     (file "~/documents/org/templates/logbook/new-logbook.org")
     :target (file+olp "~/documents/org/reports/logbook-2.org" ("Logbook"))
     :unnarrowed t)))
	 ; ("oe" "Log Quick Entry" plain
  ;   (file "~/documents/org/templates/logbook/log-entry.org")
  ;   :target (file+olp "~/documents/org/reports/logbook-2.org" ("Logbook"))
  ;   :unnarrowed t)
	 ; ("of" "Log Full Entry" plain
  ;   (file "~/documents/org/templates/logbook/log-full-entry.org")
  ;   :target (file+olp "~/documents/org/reports/logbook-2.org" ("Logbook"))
  ;   :unnarrowed t)))

;; paste logbook entries
;(defun insert-snippet ()
;  (interactive)
;  (insert-file-contents "/home/twilight/documents/org/templates/logbook/log-entry.org"))
;
;(global-set-key (kbd "C-x l") 'insert-snippet)

(defun tl/log-quick-entry ()
  (interactive)
  (let ((current-date-time (format-time-string "%Y-%m-%d %I:%M %p")))
    (end-of-line)
    (insert-file-contents "$HOME/documents/org/templates/logbook/log-entry.org")
    (next-line)
    (insert "#+DATE: " current-date-time "\n")
    (previous-line)
    (previous-line)))

(global-set-key (kbd "C-x l") 'tl/log-quick-entry)


;tl/insert-todays-date
#+end_src
