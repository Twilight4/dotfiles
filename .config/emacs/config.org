...
;; I have bound pasting from system clipboard on C-y in kitty terminal, which invokes xterm-paste command in emacs
(defun my-paste-and-replace ()
  "Paste from clipboard and replace selected region, or call xterm-paste if no region is selected."
  (interactive)
  (if (use-region-p)
      (progn
        (delete-region (region-beginning) (region-end))
...
(global-set-key (kbd "C-y") 'my-paste-and-replace)
#+end_src
